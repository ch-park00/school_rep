...
for i=2:col-1
    for j=2:row-1
        if (qtheta(i,j)==0)
            BW(i,j) = (magnitude(i,j) == ...
                max([magnitude(i,j), magnitude(i,j+1), magnitude(i,j-1)]));
        elseif (qtheta(i,j) == 1)
            BW(i,j) = (magnitude(i,j) == ...
                max([magnitude(i,j), magnitude(i+1,j-1), magnitude(i-1,j+1)]));
        elseif (qtheta(i,j) == 2)
            BW(i,j) = (magnitude(i,j) == ...
                max([magnitude(i,j), magnitude(i+1,j), magnitude(i-1,j)]));
        elseif (qtheta(i,j) == 3)
            BW(i,j) = (magnitude(i,j) == ...
                max([magnitude(i,j), magnitude(i+1,j+1), magnitude(i-1,j-1)]));
        end
...
for i = 1  : col
    for j = 1 : row
        ...
        elseif ( BW(i+1,j)>T_max || BW(i-1,j)>T_max ||BW(i,j+1)>T_max...
                || BW(i,j-1)>T_max ||BW(i-1, j-1)>T_max|| BW(i-1, j+1)...
                >T_max || BW(i+1, j+1)>T_max || BW(i+1, j-1)>T_max)
            edge_final(i,j) = 1;
        end
    end
end
...
[lines] = houghlines(output_img,T,R,P,'FillGap',10,'MinLength', 2);
max_len = 0;
